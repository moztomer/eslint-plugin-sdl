        ...
        return {
            "Literal"(node) {
                if (typeof node.value === "string") {
                    ...
                    else if (matches(blocklist, node.value) && !matches(exceptions, node.value) && shouldFix(context, node)) {
                      context.report({
                        node: node,
            ...
            "TemplateElement"(node) {
                if (typeof node.value.raw === "string" && typeof node.value.cooked === "string") {
                  ...
                  if (shouldFix(context, node) && (matches(blocklist, rawStringText) && !matches(exceptions, rawStringText)) ||
                      ...
                      context.report({
                        ...
                        fix(fixer) {
                          ...
                          if (node.value.raw.toLowerCase().includes("http")) {
                            let escapedString =  JSON.stringify(context.getSourceCode().getText(node)).replaceAll('"', ""); // delete just last and first
                            let fixedString = escapedString.replace(/http:/gi, "https:");
                            //insert an "s" before ":/" to change http:/ to https:/
