...
"use strict";
/* eslint-disable mozilla/no-insecure-url */
//------------------------------------------------------------------------------
// Rule Definition
//------------------------------------------------------------------------------
            ...
            context.report({
              ...
              fix(fixer) {
                ...
                if (node.value.toLowerCase().includes("http")) {
                  let fixedString = node.value.replace(/http:/gi, "https:");
        ...
        if (
          ...
          if (
            ...
            context.report({
              ...
              messageId: "doNotUseInsecureUrl",
              fix(fixer) {
                // Workaround for https://github.com/eslint/eslint/issues/16061
                // Only fix if it contains an http url
                if (node.value.raw.toLowerCase().includes("http")) {
                  let escapedString = JSON.stringify(node.value.raw).replaceAll('"', ""); // delete just last and first
                  let templateString = `\`${escapedString}\``;

                  let sourceCode = context.getSourceCode();
                  let text = sourceCode.getText(node);
                  let regexedString = templateString;
                  // Check if source code text includes a substitute.
                  // Since eslint is removing the '${' and inserting a ` instead remove them
                  // but ensure that the first one will still exist.
                  if (text.includes("${")) {
                    regexedString = (templateString.replaceAll("`", "${")).replace("${", "`");
                  }
                  let fixedString = regexedString.replace(/http:/gi, "https:");
                  //insert an "s" before ":/" to change http:/ to https:/
