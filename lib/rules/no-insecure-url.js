        ...
        if (
          ...
          if (
            ...
            context.report({
              ...
              messageId: "doNotUseInsecureUrl",
              fix(fixer) {
                let escapedString = JSON.stringify(node.value.raw).replaceAll('"', "");
                let templateString = `\`${escapedString}\``;
                // replace second occurrences of ` because there should be our sub
                let regexedString = (templateString.replaceAll("`", "${")).replace("${", "`");
                let fixedString = regexedString.replace("http://", "https://");
                //insert an "s" before ":/" to change http:/ to https:/
